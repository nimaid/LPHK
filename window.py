...
class Main_Window(tk.Frame):
    ...
    def init_window(self):
        ...
        display_info = lambda: self.popup(self, "About LPHK", self.about_image,
                                          "A Novation Launchpad Macro Scripting System\nMade by Ella Jameson (nimaid)\n\nVersion: " + VERSION + "\nFile format version: " + files.FILE_VERSION,
                                          "Done")
                    ...
                    if self.last_clicked is None:
                        ...
                        if self.button_mode == "move":
                            if scripts.is_bound(column, row) and ((self.last_clicked) != (column, row)):
                                self.popup_choice(self, "Button Already Bound", self.warning_image,
                                                  "You are attempting to move a button to an already\nbound button. What would you like to do?",
                                                  [["Overwrite", move_func], ["Swap", swap_func], ["Cancel", None]])
                            else:
                                move_func()
                        elif self.button_mode == "copy":
                            if scripts.is_bound(column, row) and ((self.last_clicked) != (column, row)):
                                self.popup_choice(self, "Button Already Bound", self.warning_image,
                                                  "You are attempting to copy a button to an already\nbound button. What would you like to do?",
                                                  [["Overwrite", copy_func], ["Swap", swap_func], ["Cancel", None]])
                            else:
                                copy_func()
                        elif self.button_mode == "swap":
                            swap_func()
    ...
    def draw_button(self, column, row, color="#000000", shape="square"):
        ...
        if shape == "square":
            ...
            return self.c.create_oval(x_start + shrink, y_start + shrink, x_end - shrink, y_end - shrink, fill=color,
                                      outline="")
    ...
    def draw_canvas(self):
        if self.last_clicked is not None:
            if self.outline_box is None:
                ...
                if (self.last_clicked[1] == 0) or (self.last_clicked[0] == 8):
                    self.outline_box = self.c.create_oval(x_start + (gap // 2), y_start + (gap // 2),
                                                          x_end - (gap // 2), y_end - (gap // 2), fill=SELECT_COLOR,
                                                          outline="")
                else:
                    self.outline_box = self.c.create_rectangle(x_start, y_start, x_end, y_end, fill=SELECT_COLOR,
                                                               outline="")
        ...
        if self.grid_drawn:
            ...
            self.grid_rects[8][0] = self.c.create_text(text_x, text_y, text=self.button_mode.capitalize(),
                                                       font=("Courier", BUTTON_SIZE // 3, "bold"))
    ...
    def script_entry_window(self, x, y, text_override=None, color_override=None):
        ...
        def validate_func():
            ...
            try:
                ...
                self.popup(w, "Script Validation Error", self.error_image,
        ...
        def ask_color_func():
            color = self.ask_color(w, color_button, x, y, colors_to_set[x][y])
            colors_to_set[x][y] = color
        ...
        def ask_active_color_func():
            color = self.ask_color(w, active_color_button, x, y, active_colors_to_set[x][y])
            active_colors_to_set[x][y] = color

        active_color_button = tk.Button(w, text="Select Active Color", command=ask_active_color_func)
    ...
    def ask_color(self, window, button, x, y, default_color):
        ...
        if lp_mode == "Mk1":
            color = self.classic_askcolor(color=tuple(default_color),
                                          title="Select Color for Button (" + str(x) + ", " + str(y) + ")")
        else:
    ...
    def save_script(self, window, x, y, script_text, open_editor=False, color=None):
        ...
        try:
            ...
            self.popup(window, "Script Validation Error", self.error_image,
                       "Fatal error while attempting to validate script.\nPlease see LPHK.log for more information.",
                       "OK", end_command=open_editor_func)
            raise
        if script_validate == True:
            if script_text != "":
                ...
                self.popup(window, "No Script Entered", self.info_image, "Please enter a script to bind.", "OK",
                           end_command=open_editor_func)
        else:
            self.popup(window, "(" + str(x) + ", " + str(y) + ") Syntax Error", self.error_image,
                       "Error in line: " + script_validate[1] + "\n" + script_validate[0], "OK",
                       end_command=open_editor_func)
    ...
    def modified_layout_save_prompt(self):
        if files.layout_changed_since_load == True:
            ...
            if not layout_empty:
                self.popup_choice(self, "Save Changes?", self.warning_image,
                                  "You have made changes to this layout.\nWould you like to save this layout before exiting?",
                                  [["Save", self.save_layout], ["Save As...", self.save_layout_as], ["Discard", None]])
